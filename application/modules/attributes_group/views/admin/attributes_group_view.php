		...
		<div class="card">
			...
			<div class="card-header">
				<div class="row">
					...
					<div class="col-md-8">
						<h5 class="card-title mb-0"><?php echo t('attributesgroup'); ?></h5>
					</div>
							...
							<table class="table table-striped table-hover">
								...
								<thead>
									<tr>
										...
										<th style="width:80px;" class="text-center"><?php echo t('totalattr'); ?></th>
									</tr>
								</thead>
								<tbody>
									<?php 
									...
									foreach ($data AS $r){
                                        $label = t( array( 'table'=>'attribute_group', 'field'=>'attrgroupLabel', 'id'=>$r['attrgroupId']) );
									?>
									<tr>
										...
										<td>
											...
											<div class="btn-group btn-group-xs">
												<?php if(is_edit()){ ?>
												<a href="<?php echo admin_url($this->uri->segment(2).'/edit/'.$r['attrgroupId']); ?>" class="btn btn-sm btn-info"><i class="fe fe-edit"></i> <?php echo t('edit'); ?></a> 
												<?php } 
												if(is_delete()){ ?>
												...
												<!-- Modal -->
												<div class="modal fade" id="myModal<?php echo $r['attrgroupId']; ?>" tabindex="-1" role="dialog" aria-hidden="true">
													<div class="modal-dialog" style="width:400px;">
														<div class="modal-content">
															...
															<div class="modal-body text-center">
																...
																<strong><?php echo t('attributesgroup').': '.$label; ?></strong>
															</div>
															<div class="modal-footer">
																<button type="button" class="btn btn-default btn-sm" data-dismiss="modal"><?php echo t('cancel'); ?></button>
																<a class="btn btn-danger btn-sm" href="<?php echo admin_url($this->uri->segment(2).'/delete/'.$r['attrgroupId']); ?>"><i class="icon_trash_alt"></i> <?php echo t('delete'); ?></a>
															</div>
														</div><!-- /.modal-content -->
													</div>
													<!-- /.modal-dialog -->
										...
										<td class="text-center">
											<?php echo $r['attrgroupSorting']; ?>
										</td>
										<td class="text-center">
                                            <?php
                                            $tablerelationship = array('attribute_relationship a', 'attribute b');
                                            // count group
                                            $checkgroup = countdata($tablerelationship,"a.attrgroupId='{$r['attrgroupId']}' AND b.attrId=a.attrId AND b.attrDeleted=0");
                                            if( $checkgroup > 0 ){
                                                echo $checkgroup;
                                            } else {
                                                echo '0';
