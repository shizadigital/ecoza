<?php
$maindir =  dirname( dirname( dirname( dirname( dirname( dirname(__FILE__) ) ) ) ) )  . '/index.php';
include( $maindir );
ob_end_clean();
...
if (
    empty($CI->session->userdata('username')) OR 
    empty($CI->session->userdata('passuser')) OR 
    empty($CI->session->userdata('checkpoint'))
){
    show_404();
}
else {
...
setlocale(LC_CTYPE, t('locale')); //correct transliteration
/*
|--------------------------------------------------------------------------
| Optional security
|--------------------------------------------------------------------------
...
$baseuri = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == "on") ? "https" : "http"). "://". @$_SERVER['HTTP_HOST'] . dirname( dirname( dirname( dirname( str_replace( basename( $_SERVER['SCRIPT_NAME'] ), "", $_SERVER['SCRIPT_NAME'] ) ) ) ) );
...
$config = array(
    ...
    */
    ...
    //'ftp_temp_folder'  => UPLOADS_PATH.'/temp/',
    ...
    |---------------------------------------------------------------------------
    */
    'ftp_thumbs_dir' => '../../../../thumbs/ftp/thumbs/',
    'ftp_ssl' => false,
    'ftp_port' => 21,
