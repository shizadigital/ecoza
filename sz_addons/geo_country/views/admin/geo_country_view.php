							...
							<table class="table table-striped table-hover">
								...
								<thead>
									<tr>
										...
										<th style="width:80px;" class="text-center">ISO 2</th>
										<th style="width:80px;" class="text-center">ISO 3</th>
									...
									<tr>
										...
										<td class="text-center">
											<div class="form-group">
												...
												<input type="hidden" name="item_val[<?php echo $no; ?>]" value="<?php echo $r['countryId']; ?>" />
											</div>
										</td>
										<?php } ?>
										<td class="text-center"><?php echo $no; ?></td>
										<td>
											<?php echo $r['countryName']; ?><br/>
											<div class="btn-group btn-group-xs">
												<?php if(is_edit()){ ?>
												<a href="<?php echo admin_url($this->uri->segment(2).'/edit/'.$r['countryId']); ?>" class="btn btn-sm btn-info"><i class="fe fe-edit"></i> <?php echo t('edit'); ?></a> 
												<?php } 
												if(is_delete()){ ?>
												<a data-toggle="modal" href="#myModal<?php echo $r['countryId']; ?>" class="btn btn-sm btn-danger"><i class="fe fe-trash"></i> <?php echo t('delete'); ?></a>
												<?php
												modalDelete(
													'myModal'.$r['countryId'],
													'<strong>'.$r['countryName'].'</strong>',
													admin_url($this->uri->segment(2).'/delete/'.$r['countryId'])
												);
												?>
												<?php } ?>
											</div>
										</td>
										<td class="text-center"><?php echo $r['countryIsoCode2']; ?></td>
										<td class="text-center"><?php echo $r['countryIsoCode3']; ?></td>
										<td class="text-center">
                                        <?php 
                                            echo ($r['countryStatus']==1) ? '<i class="fe fe-check text-success"></i>':'<i class="fe fe-minus"></i>';
                                        ?>
                                        </td>
									</tr>
									<?php $no++; } ?>
